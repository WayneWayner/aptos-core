...
use storage_interface::get_state_value_resolver_for_version;
...
fn test_reconfiguration() {
    ...
    let validator_account_state_view = AccountStateView {
        account_address: &validator_account,
        state_value_resolver: get_state_value_resolver_for_version(
            db.reader.clone(),
            current_version,
        ),
    };

    let root_account_state_view = AccountStateView {
    ...
    let root_account_state_view2 = AccountStateView {
        account_address: &aptos_root_address(),
        state_value_resolver: (|x| {
            let state_value_with_proof = db.reader.get_state_value_with_proof(
                x.clone(),
                current_version,
                current_version,
            )?;
            if let Some(state_value) = state_value_with_proof.value {
